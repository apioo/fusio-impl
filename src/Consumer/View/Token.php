...
class Token extends ViewAbstract
{
    public function getCollection(int $categoryId, int $userId, int $startIndex = 0)
    {
        ...
        $condition->equals(Table\Generated\AppTokenTable::COLUMN_USER_ID, $userId);
        ...
        $definition = [
            ...
            'entry' => $builder->doCollection([$this->getTable(Table\App\Token::class), 'findAll'], [$condition, $startIndex, $count, null, OrderBy::DESC], [
                ...
                'expireDate' => $builder->fieldDateTime(Table\Generated\AppTokenTable::COLUMN_EXPIRE),
                'insertDate' => $builder->fieldDateTime(Table\Generated\AppTokenTable::COLUMN_DATE),
            ]),
        ];
