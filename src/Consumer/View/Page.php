...
class Page extends ViewAbstract
{
    public function getCollection(QueryFilter $filter, ContextInterface $context): mixed
    {
        ...
        $definition = [
            'kind' => $builder->fieldValue(Service\TypeSystem\KindBuilder::build(Model\Consumer\PageCollection::class)),
            ...
            'entry' => $builder->doCollection([$this->getTable(Table\Page::class), 'findAll'], [$condition, $startIndex, $count, $sortBy, $sortOrder], [
                'kind' => $builder->fieldValue(Service\TypeSystem\KindBuilder::build(Model\Consumer\Page::class)),
    ...
    public function getEntity(string $pageId, ContextInterface $context): mixed
    {
        $builder = new Builder($this->connection);
