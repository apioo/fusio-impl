...
class Entity extends BackendApiAbstract
{
    ...
    {
        $audit = $this->tableManager->getTable(View\Audit::class)->getEntity(
            (int) $context->getUriFragment('audit_id')
        );

        if (!empty($audit)) {
            return $audit;
        } else {
            throw new StatusCode\NotFoundException('Could not find audit');
        }
    }
}
