...
class Entity extends BackendApiAbstract
{
    ...
    public function getDocumentation(?string $version = null): ?SpecificationInterface
    {
        ...
        $path = $builder->setPathParameters('Event_Entity_Path');
        $path->addInteger('event_id');

        $get = $builder->addMethod('GET');
        $get->setSecurity(Authorization::BACKEND, ['backend.event']);
        $get->addResponse(200, Model\Event::class);

        $put = $builder->addMethod('PUT');
        $put->setSecurity(Authorization::BACKEND, ['backend.event']);
        $put->setRequest(Model\Event_Update::class);
        $put->addResponse(200, Model\Message::class);

        $delete = $builder->addMethod('DELETE');
        $delete->setSecurity(Authorization::BACKEND, ['backend.event']);
