...
class Entity extends BackendApiAbstract
{
    ...
    public function getDocumentation(?string $version = null): ?SpecificationInterface
    {
        ...
        $path = $builder->setPathParameters('Event_Subscription_Entity_Path');
        $path->addInteger('subscription_id');

        $get = $builder->addMethod('GET');
        $get->setSecurity(Authorization::BACKEND, ['backend.subscription']);
        $get->addResponse(200, Model\Event_Subscription::class);

        $put = $builder->addMethod('PUT');
        $put->setSecurity(Authorization::BACKEND, ['backend.subscription']);
        $put->setRequest(Model\Event_Subscription_Update::class);
        $put->addResponse(200, Message::class);

        $delete = $builder->addMethod('DELETE');
        $delete->setSecurity(Authorization::BACKEND, ['backend.subscription']);
