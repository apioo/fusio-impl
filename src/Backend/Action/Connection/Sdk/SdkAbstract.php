 ...
 */
readonly abstract class SdkAbstract implements ActionInterface
{
    public function __construct(private Connector $connector, private FrameworkConfig $frameworkConfig)
    {
    }

    protected function getConnection(RequestInterface $request): ClientAbstract
    {
        ...
        if (!$connection instanceof ClientAbstract) {
    ...
    protected function assertConnectionEnabled(): void
    {
        if (!$this->frameworkConfig->isConnectionEnabled()) {
            throw new StatusCode\ServiceUnavailableException('SDK is not enabled, please change the setting "fusio_connection" at the configuration.php to "true" in order to activate the SDK');
        }
    }
}
