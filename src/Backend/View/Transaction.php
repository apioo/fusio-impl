...
class Transaction extends ViewAbstract
{
    public function getCollection(TransactionQueryFilter $filter, ContextInterface $context)
    {
        ...
        $condition = $filter->getCondition($this->getTable(Table\Transaction::class), [QueryFilter::COLUMN_SEARCH => Table\Generated\TransactionColumn::TRANSACTION_ID, DateQueryFilter::COLUMN_DATE => Table\Generated\TransactionTable::COLUMN_INSERT_DATE]);
        $condition->equals(Table\Generated\TransactionTable::COLUMN_TENANT_ID, $context->getTenantId());

        $builder = new Builder($this->connection);

        $definition = [
            '@type' => $builder->fieldValue(Service\JsonLD\TypeBuilder::build(Model\Backend\TransactionCollection::class)),
            ...
            'entry' => $builder->doCollection([$this->getTable(Table\Transaction::class), 'findAll'], [$condition, $startIndex, $count, $sortBy, $sortOrder], [
                '@type' => $builder->fieldValue(Service\JsonLD\TypeBuilder::build(Model\Backend\Transaction::class)),
