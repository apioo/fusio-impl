...
class Event extends ViewAbstract
{
    public function getCollection(int $categoryId, int $startIndex, int $count, ?string $search = null, ?string $sortBy = null, ?string $sortOrder = null)
    {
        ...
        $definition = [
            ...
            'itemsPerPage' => $count,
            'entry' => $this->doCollection([$this->getTable(Table\Event::class), 'findAll'], [$startIndex, $count, $sortBy, $sortOrder, $condition], [
    ...
    {
        $definition = $this->doEntity([$this->getTable(Table\Event::class), 'find'], [$this->resolveId($id)], [
            'id' => $this->fieldInteger('id'),
            'status' => $this->fieldInteger('status'),
            'name' => 'name',
            'description' => 'description',
    ...
    {
        if (substr($id, 0, 1) === '~') {
            $row = $this->getTable(Table\Event::class)->getOneByName(urldecode(substr($id, 1)));
            return $row['id'] ?? 0;
        } else {
            return (int) $id;
        }
