...
class Log extends ViewAbstract
{
    public function getCollection(LogQueryFilter $filter, ContextInterface $context): mixed
    {
        ...
        $condition = $filter->getCondition($this->getTable(Table\Log::class), [QueryFilter::COLUMN_SEARCH => Table\Generated\LogColumn::PATH, DateQueryFilter::COLUMN_DATE => Table\Generated\LogTable::COLUMN_DATE]);
        ...
        $definition = [
            'kind' => $builder->fieldValue(Service\TypeSystem\KindBuilder::build(Model\Backend\LogCollection::class)),
            ...
            'entry' => $builder->doCollection([$this->getTable(Table\Log::class), 'findAll'], [$condition, $startIndex, $count, $sortBy, $sortOrder], [
                'kind' => $builder->fieldValue(Service\TypeSystem\KindBuilder::build(Model\Backend\Log::class)),
