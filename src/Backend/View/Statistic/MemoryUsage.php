...
use Fusio\Impl\Backend\View\System\Usage\QueryFilter;
 ...
 */
class MemoryUsage extends ViewAbstract
{
    public function getView(QueryFilter $filter)
    {
        ...
        // fill values
        $sql = '  SELECT AVG(usag.memory_usage) AS avg_usage,
                         DATE(usag.insert_date) AS date
                    FROM fusio_system_usage usag
                   WHERE ' . $expression . '
        ...
        foreach ($result as $row) {
            if (isset($data[$row['date']])) {
                $data[$row['date']] = (int) $row['avg_usage'];
            }
        }
