...
class Category extends ViewAbstract
{
    public function getCollection(int $startIndex, int $count, ?string $search = null)
    {
        ...
        $definition = [
            ...
            'itemsPerPage' => $count,
            'entry' => $this->doCollection([$this->getTable(Table\Category::class), 'getAll'], [$startIndex, $count, 'name', Sql::SORT_ASC, $condition], [
    ...
    {
        $definition = $this->doEntity([$this->getTable(Table\Category::class), 'get'], [$id], [
            'id' => $this->fieldInteger('id'),
            'status' => $this->fieldInteger('status'),
            'name' => 'name',
        ]);
