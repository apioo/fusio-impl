...
use Fusio\Impl\Factory\Resolver\JavascriptFile;
...
/**
 * DeployCommandRouteFileJsTest
 *
 * @author  Christoph Kappestein <christoph.kappestein@gmail.com>
 * @license http://www.gnu.org/licenses/agpl-3.0
 * @link    http://fusio-project.org
 */
class DeployCommandRouteFileJsTest extends ControllerDbTestCase
{
    ...
    public function testCommandRoutesActionClass()
    {
        ...
        $commandTester->execute([
            ...
            'file'    => __DIR__ . '/../resource/deploy_routes_action_file_js.yaml',
        ]);

        $display = $commandTester->getDisplay();

        $this->assertRegExp('/- \[CREATED\] action s_Console_System_resource_test-action_js/', $display, $display);
        ...
        $action = $this->connection->fetchAssoc('SELECT id, class, engine, config FROM fusio_action WHERE name = :name', [
            'name' => 's_Console_System_resource_test-action_js',
        ]);

        $this->assertEquals(5, $action['id']);
        $this->assertContains('Console/System/resource/test-action.js', $action['class']);
        $this->assertEquals(JavascriptFile::class, $action['engine']);
    ...
    public function testCommandRoutesActionClassInvalid()
    {
        ...
        $commandTester->execute([
            'command' => $command->getName(),
            'file'    => __DIR__ . '/../resource/deploy_routes_action_file_js_invalid.yaml',
        ]);

        $display = $commandTester->getDisplay();

        $this->assertRegExp('/Could not resolve action foo_js/', $display, $display);
    }
}
