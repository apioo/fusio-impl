<!DOCTYPE>
<html lang="en">
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1">
  <title>Fusio - Authorize</title>
  <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.6/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-4Q6Gf2aSP4eDXB8Miphtr37CMZZQ5oXLH2yaXMJ2w8e2ZtHTl7GptT4jmndRuHDT" crossorigin="anonymous">
</head>
<body>

<div class="container" style="margin-top:64px;max-width:640px">
  <div class="text-center">
    <img src="data:image/gif;base64,">
  </div>
  <div class="mt-5 pb-5 mb-3 border-bottom">
    <?php if (isset($error)): ?>
      <div class="alert alert-danger">
        <?php echo $error; ?>
      </div>
    <?php elseif (!empty($code)): ?>
      <p>The app has requested an access token, but no redirect uri was provided, you can provide the following code to
      the app to complete the authorization request:</p>
      <pre>Code: <b><?php echo $code; ?></b></pre>
    <?php elseif (!empty($type)): ?>
      <p>The app has requested an access token, but no redirect uri was provided and we could also not generate a code
      for this app. We got the following error type: <b><?php echo $type; ?></b></p>
    <?php else: ?>
      <p>The app <b><?php echo $app->name; ?></b> wants to access your account, it requests
      the following scopes:</p>
      <dl>
        <?php foreach ($app->scopes ?? [] as $scope): ?>
        <dt><?php echo $scope->name; ?></dt>
        <dd><?php echo $scope->description; ?></dd>
        <?php endforeach; ?>
      </dl>
      <?php if (!empty($app->url)): ?>
      <p><small>App-Url: <a href="<?php echo $app->url; ?>"><?php echo $app->url; ?></a></small></p>
      <?php endif; ?>
      <hr>
      <p>Please authenticate to authorize this request:</p>
      <form method="POST">
        <div class="mb-3">
          <label for="username" class="form-label">Username</label>
          <input type="text" id="username" name="username" class="form-control">
        </div>
        <div class="mb-3">
          <label for="password" class="form-label">Password</label>
          <input type="password" id="password" name="password" class="form-control">
        </div>
        <button type="submit" class="btn btn-primary">Authorize</button>
      </form>
    <?php endif; ?>
  </div>
  <p class="text-muted text-end"><small>powered by <a href="https://www.fusio-project.org/">Fusio</a>.</small></p>
</div>

</body>
</html>
